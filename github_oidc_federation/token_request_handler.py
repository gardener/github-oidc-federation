...
import github
from .github_api import fetch_installation_token
...
GITHUB_API_LOOKUP: github.GithubApiLookup | None = None
...
async def request_token(request: aiohttp.web.Request) -> aiohttp.web.Response:
    ...
    oidc_federation_config = retrieve_oidc_federation_config(token_request)

    claims = verify_jwt(token_request.jwt, token_request.issuer)
    ...
    result = fetch_installation_token(token_request, claims['sub'])
...
def _resolve_request(body: dict) -> TokenRequest:
    ...
    try:
        api = GITHUB_API_LOOKUP(repo_url)
...
def _check_matching_repositories(
    ...
    if entry.repositories:
        if not token_request.requested_repositories:
            logger.warning('entry specifies repositories but request does not specify any repositories')
            return False

        requested_repositories_match_entry_repositories = all(
            repository in entry.repositories
            for repository in token_request.requested_repositories
        )
...
def _check_permissions(
    ...
    for permission, requested_level in token_request.permissions.items():
        ...
        if not allowed_level or PermissionLevel(requested_level) > allowed_level:
            logger.warning(
